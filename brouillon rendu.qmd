...
# Indice moyen d'éloignement par region ?

Eloi2024_Reg <- IVAL_IPS_gt |> group_by(Region)  |> summarize(moyenne_eloignement_2024 = mean(`Indice éloignement_2024`, na.rm = TRUE)) 
...
```{r}
# Indice moyen d'eloignement par Region (geoplot) ?
...
tm_shape(region_eloi) + tm_fill(fill="moyenne_eloignement_2024",
                             fill.scale=tm_scale_intervals(values="reds", n=5,
                                                           style="quantile"))
...
```{r}
# IVAL selon les regions 2016 (geoplot) ?
# ca fonctionne pas encore

st_layers("France_entiere.gpkg")
region <- st_read("France_entiere.gpkg", layer="region")
region <- region |> rename(Region="nom_officiel_en_majuscules")
IVAL_2016 <- IVAL_IPS_gt |> select(IVAL_2016, Region)
IVAL_regions <- region |> select(Region, code_insee, code_siren, geometrie) |> right_join(IVAL_2016, by="Region")


tm_shape(IVAL_regions) + tm_fill(fill="IVAL_2016")
